...
let savedStocksWatchlistEl = document.querySelector(
  "#new-saved-stocks-watchlist"
);
...
let parentNewsContainerEl = document.querySelector(
  "#png-latest-news-container"
);
...
async function getTrendingNews(dateOf) {
  let trendingStocksURL = `https://api.marketaux.com/v1/news/all?&language=en&countries=us&language=en&published_after=${dateOf}&page=${getRandomInt(
    1,
    100
...
var createNewsCards = function (newsArticles) {
  ...
  if (imageLoaded(imageEl)) {
    ...
    // Otherwise, make it the stock image
    imageEl.setAttribute(
      "src",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQRfcFnHFv_0MtuEj0kDcrrefVzmL_oxTsDKw&usqp=CAU"
    );
  }
  ...
  if (newsArticles["description"] === "") {
    ...
    // Otherwise, use the API's description for the article
...
async function getBasicFinancial() {
  ...
  marketCapNumberEl.textContent = `${abbreviateNumber(
    Math.round(data.metric.marketCapitalization) * 1000000,
    2,
    2,
    false
  )}`;
  ...
  if (data.metric.dividendYieldIndicatedAnnual === null) {
    ...
    yieldHeaderEl.textContent = "Yield";
    yieldNumberEl.textContent =
      data.metric.dividendYieldIndicatedAnnual.toFixed(2);
  }
...
async function getFacebookStockQuote() {
  let stockQuoteUrl = `https://finnhub.io/api/v1/quote?symbol=META&token=c7tl5miad3i8dq4u5t50`;
  let response = await fetch(stockQuoteUrl);
  let data = await response.json();
